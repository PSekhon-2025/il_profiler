...
- **Hallucination** — the five opt-in checks for any saved run: alert banners
  ...
  metamorphic label-stability eval (launchable from this tab, flagged items
  shown variant-by-variant), and the embedding-agreement check (binary +
  graded closeness metrics, launchable from this tab).
...
verdict), lab-swap substitution, and the stability math.
...
`--sample` first. Quote provenance is cheapest-first and short-circuits: a run
whose quotes are all verbatim costs nothing, and otherwise it is one chat call
per non-verbatim span plus a handful of cached embeddings. Stages are
resumable, so an interrupted run loses nothing.
